...
- All nine bundled plugins (brave-free, ddgs, searxng, exa, parallel,
  tavily, firecrawl, sub2api-web-search, xai) instantiate and self-report the expected
...
def _clear_web_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for k in (
        ...
        "OPENAI_API_KEY",
        "SUB2API_WEB_SEARCH_BASE_URL",
        "SUB2API_WEB_SEARCH_API_KEY",
        "SUB2API_WEB_SEARCH_MODEL",
        "XAI_API_KEY",
    ):
...
class TestBundledPluginsRegister:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("tavily", True, True),
            ("firecrawl", True, True),
            # Sub2API: search-only through an OpenAI-compatible /responses endpoint.
            ("sub2api-web-search", True, False),
